...
```
![Sample panel plot](\instructions_fig\Total numbers of steps per day.png) 
...
```
![Sample panel plot](\instructions_fig\Average steps for each 5-min interval.png)
...
```
![Sample panel plot](\instructions_fig\Total number of steps taken each day(No NA).png)
...
ggplot(dtIntervals, aes(x=interval, y=meanSteps, color=dayType)) +
  ...
  theme_bw()
```
![Sample panel plot](\instructions_fig\Weekday and Weekend comparement.png)
